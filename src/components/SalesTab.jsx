  ...
  return (
    <div className="card">
            ...
            {rows.map((r, i) => {
              ...
              return (
                <tr key={r.id}>
                  ...
                  <td><input className="cell-inp" placeholder="Товар / услуга" value={r.name} onChange={e => onUpdate(r.id,'name',e.target.value)} /></td>
                  <td><input className="cell-inp" placeholder="Модель, тип..." value={r.type} onChange={e => onUpdate(r.id,'type',e.target.value)} /></td>
                  <td><input className="num-inp" type="number" placeholder="0" value={r.total} onChange={e => onUpdate(r.id,'total',e.target.value)} /></td>
                  <td className="hide-mobile"><input className="num-inp" type="number" placeholder="0" value={r.cost} onChange={e => onUpdate(r.id,'cost',e.target.value)} /></td>
      ...
      <div className="summary s3">
        <div className="sum-card"><div className="s-label">Выручка</div><div className="s-val s-blue">{fmt(totals.t)}</div></div>
        <div className="sum-card"><div className="s-label">Себестоимость</div><div className="s-val s-gray">{fmt(totals.c)}</div></div>
        <div className="sum-card">
          <div className="s-label">Прибыль</div>
          <div className={`s-val ${totals.p>=0?'s-green':'s-red'}`}>{(totals.p>=0?'+':'')+fmt(totals.p)}</div>
        </div>
      </div>
    </div>
  )
